...
def test_create_crime_report(token):
    ...
    headers = {'Authorization': f'Bearer {token}'}
    response = requests.post(f'{BASE_URL}/crime-reports/', json=TEST_CRIME_REPORT, headers=headers)
    print(f"Status Code: {response.status_code}")
    print(f"Response: {response.json()}")
    return response.json() if response.status_code == 201 else None
...
def main():
    ...
    if police_data:
        ...
        if police_token:
            ...
            # Test creating police station
            station_data = test_create_police_station(police_token, police_data['id'])
    
    # Test department registration and login
    ...
    # If we have a report ID, test updating its status
